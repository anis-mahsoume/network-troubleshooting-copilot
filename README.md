...
![Chat tab diagnosing a DHCP lease failure](docs/troubleshooting_chat_diagnosis.png)
...
The repo ships with a pre-ingested knowledge base (`data/processed/chunks.json`), so you can run the app right away without touching any Python scripts:

```bash
python -m streamlit run app.py
```

It opens with three tabs:

- **Ingest**: shows which PDFs under `data/raw/` are already ingested vs. pending, lets you run ingestion on pending files from the UI, and lets you browse the resulting chunks.
- **Retrieve**: run ad-hoc queries against the knowledge base and inspect the top-k matches with similarity scores.
- **Chat**: retrieves relevant documentation for your message, then lets the model call mocked diagnostic tools (ping, traceroute, device status, DNS lookup, DHCP lease check) until it has enough information to answer. Comes with a scenario picker, and shows retrieved sources and tool calls alongside the final diagnosis.

## Under the hood: CLI usage

The Streamlit app is a thin UI over a few standalone pieces, all runnable on their own.
...
data/
  ...
  processed/
    chunks.json       generated output: chunked docs + embeddings
rag/
